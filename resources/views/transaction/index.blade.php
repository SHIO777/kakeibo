                ...
                <td class="py-4 px-6 border-b border-grey-light">
                  ...
                  <div class="flex">
                    ...
                    <form action="{{ route('transaction.destroy',$transaction->id) }}" method="POST" class="text-left">
                      @method('delete')
                      @csrf
                      <button type="submit" class="mr-2 ml-2 text-sm hover:bg-gray-200 hover:shadow-none text-white py-1 px-2 focus:outline-none focus:shadow-outline" onclick="return confirm('Are you sure you want to delete the transaction of ¥{{ $transaction->price }} on {{ $transaction->date }}?')">
                        <svg class="h-6 w-6 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="black">
